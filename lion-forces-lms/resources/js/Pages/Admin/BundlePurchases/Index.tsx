...
export default function BundlePurchasesIndex({ purchases, filters }: Props) {
    return (
        <AdminLayout header="Bundle Purchases">
            <Head title="Bundle Purchases" />

            <div className="mb-4 flex gap-2">
                {[null, 'pending', 'verified', 'rejected'].map((s) => (
                    <button
                        key={s ?? 'all'}
                        onClick={() => router.get('/admin/bundle-purchases', { status: s ?? undefined }, { preserveState: true })}
                    ...
                    <tbody className="divide-y divide-border">
                        {purchases.data.map((p) => (
                            <tr key={p.id}>
                                ...
                                <td className="px-5 py-3 text-text-secondary">{p.bundle?.title}</td>
                                ...
                                <td className="px-5 py-3">
                                    {p.proof_file_path ? (
                                        <a href={`/storage/${p.proof_file_path}`} target="_blank" rel="noopener noreferrer" className="font-semibold text-primary hover:underline">
                                            View
                                        </a>
                                    ) : (
                                ...
                                <td className="px-5 py-3 text-right">
                                    {p.status === 'pending' && (
                                        <div className="flex justify-end gap-2">
                                            <button onClick={() => router.post(`/admin/bundle-purchases/${p.id}/verify`)} className="text-xs font-bold uppercase text-success hover:underline">Verify</button>
                                            <button onClick={() => router.post(`/admin/bundle-purchases/${p.id}/reject`)} className="text-xs font-bold uppercase text-danger hover:underline">Reject</button>
                                        </div>
                                    )}
                                </td>
                            </tr>
            ...
            {purchases.data.length === 0 && <p className="py-10 text-center text-text-secondary">No bundle purchases found.</p>}
