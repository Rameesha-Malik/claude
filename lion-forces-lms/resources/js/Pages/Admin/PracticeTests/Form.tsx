...
export default function PracticeTestForm({ course, practiceTest, subjects, questionBank }: Props) {
    const isEdit = !!practiceTest;
    const [subjectFilter, setSubjectFilter] = useState<string>('');

    const form = useForm({
        title: practiceTest?.title ?? '',
        timer_enabled: practiceTest?.timer_enabled ?? false,
        duration_minutes: practiceTest?.duration_minutes ?? '',
        question_selection_mode: practiceTest?.question_selection_mode ?? 'manual',
        subject_id: practiceTest?.subject_id ?? '',
        auto_question_count: practiceTest?.auto_question_count ?? '',
        shuffle_questions: practiceTest?.shuffle_questions ?? true,
        marks_per_question: practiceTest?.marks_per_question ?? '1',
        negative_marking: practiceTest?.negative_marking ?? '0',
        is_repeatable: practiceTest?.is_repeatable ?? true,
        is_active: practiceTest?.is_active ?? true,
        question_ids: practiceTest?.questions?.map((q) => q.id) ?? [] as number[],
    });
    ...
    function submit(e: React.FormEvent) {
        e.preventDefault();
        if (isEdit) form.put(`/admin/courses/${course.id}/practice-tests/${practiceTest!.id}`);
        else form.post(`/admin/courses/${course.id}/practice-tests`);
    }

    const filteredBank = subjectFilter ? questionBank.filter((q) => String(q.subject_id) === subjectFilter) : questionBank;

    return (
        <AdminLayout header={isEdit ? 'Edit Practice Test' : 'New Practice Test'}>
            ...
            <form onSubmit={submit} className="grid gap-6 lg:grid-cols-3">
                <div className="space-y-4 rounded-2xl border border-border bg-surface p-6 lg:col-span-1">
                    ...
                    <label className="flex items-center gap-2 text-sm text-text">
                        <input type="checkbox" checked={form.data.timer_enabled} onChange={(e) => form.setData('timer_enabled', e.target.checked)} />
                        Timer enabled
                    </label>
                    {form.data.timer_enabled && (
                        <div>
                    ...
                    <button type="submit" disabled={form.processing} className="w-full rounded-lg bg-primary py-2.5 text-sm font-bold uppercase tracking-wide text-on-primary hover:bg-primary-hover">
                        {isEdit ? 'Save Changes' : 'Create Practice Test'}
                    </button>
