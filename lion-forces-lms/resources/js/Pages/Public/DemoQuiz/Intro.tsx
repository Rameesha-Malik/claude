...
interface Quiz {
    id: number; title: string; duration_minutes: number; questions_count: number;
    subject: { name: string } | null; category: { name: string } | null;
}
interface Props { quiz: Quiz | null; quizzes: Quiz[]; pageContent: { title: string; subtitle: string } }

function QuizCard({ q }: { q: Quiz }) {
    return (
        <div className="flex flex-col rounded-2xl border border-border bg-surface p-6 text-center shadow-2xl">
            {q.subject && (
...
export default function DemoQuizIntro({ quiz, quizzes, pageContent }: Props) {
    ...
    // Client: "Demo Quizzes >>> Category >>> like Army, navy airforce >>>
    // each category have its own demos like army >>> lcc >>> pma etc" --
    // group cards under their exam category (same categories Courses use)
    // when there's real variety to group by; a flat grid (no headers)
    // covers the common single-category or all-uncategorized case so this
    // never shows a lone "Uncategorized" heading over everything.
    const categoryNames = Array.from(new Set(quizzes.map((q) => q.category?.name).filter(Boolean)));
    const grouped = categoryNames.length > 1;
    const groups = grouped
        ? [
              ...categoryNames.map((name) => ({ name: name as string, items: quizzes.filter((q) => q.category?.name === name) })),
              { name: 'Other', items: quizzes.filter((q) => !q.category) },
                ...
                <div className={hasMultiple ? 'mx-auto max-w-4xl' : 'mx-auto max-w-2xl'}>
                    {hasMultiple && grouped ? (
                        <div className="space-y-10">
                            {groups.map((g) => (
                                <div key={g.name}>
                                    <h2 className="mb-4 text-left font-display text-xl uppercase tracking-wide text-text">{g.name}</h2>
                                    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                                        {g.items.map((q) => <QuizCard key={q.id} q={q} />)}
                                    </div>
                                </div>
                            ))}
                        </div>
